...
from src.treasure_hunt import (
    ...
    create_hunt, get_hunt, update_hunt, delete_hunt, list_hunts,
    pull_card, remaining_list, render_remaining_as_text,
)
...
def _attach_hunt_images(plan):
    """Attach cached image URIs to every card in the plan."""
...
def treasure_hunt():
    if request.method == "GET":
        return render_template("treasure_hunt_form.html", hunts=list_hunts())
    ...
    include_decks = bool(request.form.get("include_decks"))
    plan = plan_treasure_hunt(wanted, include_deck_boxes=include_decks)
    _attach_hunt_images(plan)

    title = request.form.get("title", "").strip()
    if not title:
        title = f"Hunt — {datetime.now().strftime('%b %d, %Y %H:%M')}"
    hunt_id = create_hunt(title, wanted, plan)
    return redirect(url_for("treasure_hunt_view", hunt_id=hunt_id))


@app.route("/treasure-hunt/hunt/<int:hunt_id>")
def treasure_hunt_view(hunt_id):
    hunt = get_hunt(hunt_id)
    if not hunt:
        flash("That hunt doesn't exist (it may have been deleted).", "error")
        return redirect(url_for("treasure_hunt"))
    plan = hunt["plan"]
    _attach_hunt_images(plan)
    remaining = remaining_list(plan)
    return render_template(
        "treasure_hunt_result.html",
        hunt=hunt, plan=plan,
        total_unique=len(hunt["wanted"]),
        remaining_cards=sum(q for q, _ in remaining),
        remaining_unique=len(remaining),
    )


@app.route("/treasure-hunt/hunt/<int:hunt_id>/pull", methods=["POST"])
def treasure_hunt_pull(hunt_id):
    hunt = get_hunt(hunt_id)
    if not hunt:
        flash("That hunt doesn't exist (it may have been deleted).", "error")
        return redirect(url_for("treasure_hunt"))
    key = request.form.get("key", "")
    msg = pull_card(hunt["plan"], key)
    if msg is None:
        flash("Couldn't find that card in this hunt.", "error")
    else:
        update_hunt(hunt_id, plan=hunt["plan"])
        flash(msg, "success")
    return redirect(url_for("treasure_hunt_view", hunt_id=hunt_id))


@app.route("/treasure-hunt/hunt/<int:hunt_id>/rename", methods=["POST"])
def treasure_hunt_rename(hunt_id):
    title = request.form.get("title", "").strip()
    if not title:
        flash("Hunt name can't be empty.", "error")
    elif update_hunt(hunt_id, title=title):
        flash(f"Renamed hunt to '{title}'.", "success")
    else:
        flash("That hunt doesn't exist.", "error")
    return redirect(url_for("treasure_hunt_view", hunt_id=hunt_id))


@app.route("/treasure-hunt/hunt/<int:hunt_id>/delete", methods=["POST"])
def treasure_hunt_delete(hunt_id):
    delete_hunt(hunt_id)
    flash("Hunt deleted.", "success")
    return redirect(url_for("treasure_hunt"))


@app.route("/treasure-hunt/hunt/<int:hunt_id>/export")
def treasure_hunt_export(hunt_id):
    """Download the cards still needed (not pulled / not owned / shortfalls)
    as a plain decklist: '1 Card Name' per line."""
    hunt = get_hunt(hunt_id)
    if not hunt:
        flash("That hunt doesn't exist (it may have been deleted).", "error")
        return redirect(url_for("treasure_hunt"))
    text = render_remaining_as_text(hunt["plan"])
    if not text:
        flash("Nothing left to export — every card is pulled and accounted for.",
              "success")
        return redirect(url_for("treasure_hunt_view", hunt_id=hunt_id))
    ...
    safe_title = re.sub(r"[^A-Za-z0-9_-]+", "_", hunt["title"]).strip("_") or "hunt"
    fname = f"{safe_title}_still_needed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
